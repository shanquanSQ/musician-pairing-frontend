...
export function GenreList({isOwnPage, displayedUserId}) {
    ...
    useEffect(() => {
        const getGenreInfo = async () => {
            const genreInfo = await axios.get(`${BACKEND_URL}/users/${displayedUserId}/genres`)
    ...
    const writeData = async () => {
        ...
        await axios.put(`${BACKEND_URL}/users/${displayedUserId}/genres`, {genresList})
    }
    ...
    const genreText = genresList.map((genre, index) => {
        return (
          ...
          {genre.toUpperCase() + " "}
            {isBeingEdited ? 
                ...
                <button onClick={() => removeGenre(index)} id={`deleteGenre-${index}`} style={{ display: "none" }} />
            </div>
            : null}
            /&nbsp;
            </div>
      ...
      const newGenreInput =
          <div className = "flex flex-row">
              ...
              <button onClick={() => addGenre()} id={`addGenre`} style={{ display: "none" }} />
          </div>
    ...
    return (
            <div>
                <div className="flex flex-row">
                    ...
                    <button onClick={() => setIsBeingEdited(true)} id={`editButton-genres`} style={{ display: "none" }} />
                    {isBeingEdited ?
                    <div className='flex flex-row'>
